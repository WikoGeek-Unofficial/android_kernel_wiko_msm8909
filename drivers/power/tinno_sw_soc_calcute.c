...
#define  LI_POLYMER_BATTERY             0 
#define  LI_ION_BATTERY                      1
static int str_to_int(char *str);
int battery_type_init_flag=0;
...
#if defined(CONFIG_TINNO_P4901)||defined(CONFIG_TINNO_P4903)||defined(CONFIG_TINNO_P4901TK)||defined(CONFIG_TINNO_P4903JP)||defined(CONFIG_TINNO_L5251)
...
{	
	...
	}else if(battype==LI_ION_BATTERY)
	{
...
{    
    ...
    for (i = 0; i < saddles - 1; i++)
    {
        ...
        {
		if(((profile_p+i)->q_cost)== ((profile_p+i+1)->q_cost))
		{
...
int fgauge_read_r_bat_by_v_by_table(R_PROFILE_STRUC_P profile_p,int voltage){
 int i = 0, saddles = 0;
    ...
    for (i = 0; i < saddles - 1; i++)
    {
		...
		if(((profile_p+i)->resistance)== ((profile_p+i+1)->resistance))
		{
...
void fgauge_construct_battery_profile(int temperature, BATTERY_Q_COST_PROFILE_STRUC_P temp_profile_p)
{
	...
	if((temp_profile_p + i)->voltage==3075)
	{
		...
		if( ( (temperature - low_temperature) * (temp_d_1 - temp_d_2))==0)
		{
        ...
        else if( high_profile_d<low_profile_d)
        {
		...
		if( ( (high_temperature - temperature) * (temp_d_1 - temp_d_2))==0)
		{
			(temp_profile_p + i)->q_cost = temp_d_2;
		}else{
		            (temp_profile_p + i)->q_cost= temp_d_2 +
...
{    
   ...
   tinno_pr_debug("fgauge_read_v_by_Qcost :%d \n",q_cost);
    ...
    for (i = 0; i < saddles - 1; i++)
    {
        ...
        {
		if(((profile_p+i)->voltage)== ((profile_p+i+1)->voltage))
		{
...
/*use the high temp voltage*/
 for (i = 0; i < saddles; i++){
	...
	low_profile_r=fgauge_read_r_bat_by_v_by_table(low_profile_p,(temp_profile_p + i)->voltage);
		...
		if( ( (temperature - low_temperature) * (temp_r_1 - temp_r_2))==0)
		{
        ...
        else if( high_profile_r<low_profile_r)
        {
            temp_r_1 =low_profile_r;
            temp_r_2 =high_profile_r;
  		if( ( (high_temperature - temperature) * (temp_r_1 - temp_r_2))==0)
		{
			(temp_profile_p + i)->resistance = temp_r_2;
		}else{
	            (temp_profile_p + i)->resistance = temp_r_2 +
...
#ifdef CONFIG_TINNO_L5251
    int battype_temp=255;
		char * start;
		if(battery_type_init_flag==0)
		{
				start=strstr(saved_command_line,BAT_TYPE_STR);
				    if(start)
				    {
					battype_temp=get_battery_type_in_cmdline(start+strlen(BAT_TYPE_STR));
				    }
					printk("battype=%d\n",battype);
					if((battype_temp<=MAX_BATTERY_TYPE)&&(battype_temp>=0))
					{
					   battype=battype_temp;						
					}
			battery_type_init_flag=1;		
		}
#endif
...
//	int ret=0;
	...
	{
	    	current_temp = (((voltage_last-voltage)*1000)*10) /bat_r;    //0.1mA
	    	if((current_temp*maintain_times)!=0)
	    	{
...
void force_init_soc(int soc, int ocv)
{
	...
	bat_totals_columb_st=fgauge_get_Q_max_match_Current_and_temp(400,25);
	if(bat_totals_columb_st>gFG_columb_standard)
	{
		bat_totals_columb_st=gFG_columb_standard;
	}
