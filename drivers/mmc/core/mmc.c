...
static int mmc_partial_init(struct mmc_host *host)
{
	...
	pr_debug("%s: restoring cached ios, clock %uHz busmode %u width %u timing %u\n",
		mmc_hostname(host),
		host->cached_ios.clock,
		host->cached_ios.bus_mode,
		host->cached_ios.bus_width,
		host->cached_ios.timing);
...
static int mmc_resume(struct mmc_host *host)
{
	...
	while (retries) {
		if (mmc_card_can_sleep(host) &&
			...
			if (err)
				pr_err("%s: %s: partial init failed (%d)\n",
					mmc_hostname(host),  __func__, err);
		}
		if (err)
