...
int tpd_has_probe_flag=0;
static int main_clock_init_lock=0; 
...
#define VIRTUAL_TOUCH_KEY_INDEX   (GTP_MAX_TOUCH+1)
...
static  int wake_up_switch=0;
int wake_up_irq_status=0;
#if 0
#define TINNO_KEY_WAKEUP  KEY_POWER//601
#define TINNO_KEY_CAMERA  KEY_C   //602
#define TINNO_KEY_LIGHT   KEY_O   //603
#else
#define TINNO_KEY_WAKEUP   601
#define TINNO_KEY_CAMERA   602
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    s32 ret = 2;
#if 0	
    u8 buf[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
        = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
    s32 retry = 0;
#endif
 ...
 //   {
        ...
        if (ret > 0)
        {
        	ret = gtp_i2c_read(client, buf, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
             	if(ret > 0 && !memcmp(buf, config, ts->gtp_cfg_len -1 + GTP_ADDR_LENGTH)){
                 break;
             	}
        }
#endif		
...
#if GTP_ICS_SLOT_REPORT
    input_mt_init_slots(ts->pen_dev, 16,INPUT_MT_DIRECT);               // 
#else
...
//zgm add
	...
	case 0xFF: 
	case 0x00:	
		GTP_INFO("Request preparation.0x%x ",rqst_buf[2]); 
	break; 
    ...
    if(key_value || pre_key)
    {
		...
		{
			printk("FTM_MODE :report key key_value=%d,pre_key =%d ,pre_key_value =%d\n",key_value, pre_key,pre_key_value);

			if(pre_key == 0)
			{
				if(key_value&0x01)
				{//
					 input_report_key(ts->input_dev, touch_key_array[0], 1);
					 pre_key_value =0;
				}
				else if(key_value&0x02)
				{
					  input_report_key(ts->input_dev, touch_key_array[1], 1);	
					   pre_key_value =1;
				}
				else if(key_value&0x04)
				{
					 input_report_key(ts->input_dev, touch_key_array[2], 1);
					  pre_key_value =2;
				}
			}
			else
			{
				if(key_value ==0)
				{
					 input_report_key(ts->input_dev, touch_key_array[pre_key_value], 0);
				}
			}
		...
		if(boot_mode!=FTM_MODE)
		{
			if(key_value&0x01)
			{//
				gtp_touch_down(ts, VIRTUAL_TOUCH_KEY_INDEX, 400, 900, 60);
			}
			if(key_value&0x02)
			{
				 gtp_touch_down(ts, VIRTUAL_TOUCH_KEY_INDEX, 240, 900, 60);	
			}
			if(key_value&0x04)
			{
				 //
				  gtp_touch_down(ts, VIRTUAL_TOUCH_KEY_INDEX, 80, 900, 60);	
			}
			if(!key_value)
			{
				 gtp_touch_up(ts,VIRTUAL_TOUCH_KEY_INDEX);
			}
			printk("report key key_value=%d \n",key_value);
		}
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        for (retry = 0; retry < 10; ++retry)
        {
            ...
            gup_load_init_code(); //reload init code           
...
#if GTP_ICS_SLOT_REPORT
    input_mt_init_slots(ts->input_dev, 16,INPUT_MT_DIRECT);     // in case of "out of memory"
#else
...
static s32 gtp_main_clk_proc(struct goodix_ts_data *ts)
{
    ...
    static int data_path_vaild_count=0;

    ret = gup_check_fs_mounted("/data");
    if(ret)
    {
	data_path_vaild_count++;
	if(data_path_vaild_count<2)
	{
		msleep(500);
		return FAIL;
	}
    }
	GTP_INFO("/data  mounted count %d \n.",data_path_vaild_count);
    if (FAIL == ret)
    {
        ts->clk_chk_fs_times++;
...
static void set_int_pin_state(int state)
{
      ...
      if (IS_ERR(Touch_pinctrl))
      	{
		printk("gtp_parse_dt : not find pinctl!\n");
		return;
	 }
	else{
		     if(state)
			...
			if (IS_ERR(set_state)) {
				printk("cannot get  pinctrl active state\n");
				return ;
				}
		}
		...
		if(ret)
			{
			printk("set_int_pin_state ret:%d \n",ret);
		}
}
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (!prop || !prop->value || *cfg_len == 0 || *cfg_len > GTP_CONFIG_MAX_LENGTH) {
		...
		printk("prop->value[0]=0x%x \n",((char*)prop->value)[0]);
#ifdef CONFIG_TINNO_DEV_INFO		
		cfg_ver=((char*)prop->value)[0];
#endif
		return 0;
	}
}
...
#if GTP_GESTURE_WAKEUP
static ssize_t show_wake_up_switch(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct goodix_ts_data *ts = NULL;
       ts = i2c_get_clientdata(i2c_connect_client);
    	printk("[show_wake_up_switch] \n");
	...
	if(wake_up_switch)
	{
	    return sprintf(buf, "Smart Wake Up Is On. \n");
	}else{ 
	    return sprintf(buf, "Smart Wake Up Is Off. \n");
	}
}

static ssize_t store_wake_up_switch(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
...
#ifdef  CONFIG_TINNO_DEV_INFO
void update_fw_version(u16 ver)
{
	if (5 == gtp_sensor_id)
    {
...
static int tpd_registration(void*_client)
{
    ...
    if (ret < 0)
    {
		...
		sprintf(des_buf, "YEJI-GT950-L5221AE-%x-%d",version_info,cfg_ver);
	#else
//		#ifdef CONFIG_TINNO_L5251
		#if defined (CONFIG_TINNO_L5251) || defined (CONFIG_TINNO_P4901)
				sprintf(des_buf, "YEJI-GT9157-L5251AE-%x-%d",version_info,cfg_ver);
		#else
				sprintf(des_buf, "YEJI-GT950-LXXXX-%x-%d",version_info,cfg_ver);
		#endif
	#endif
	SET_DEVINFO_STR(TouchPanel,des_buf);
	sprintf(des_buf, "%x",version_info);
	SET_DEVINFO_STR(TouchPanel_Fw_Ver,des_buf);	
#endif
#else
#ifdef  CONFIG_TINNO_DEV_INFO
 update_fw_version(version_info);
#endif
#endif
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    #ifdef  CONFIG_TINNO_DEV_INFO
	#ifdef CONFIG_TINNO_L5221 
		sprintf(des_buf, "YEJI-GT950-L5221-%x-%d",version_info,cfg_ver);
	#else
//		#ifdef CONFIG_TINNO_L5251
		#if defined (CONFIG_TINNO_L5251) || defined (CONFIG_TINNO_P4901)
				sprintf(des_buf, "YEJI-GT9157-L5251-%x-%d",version_info,cfg_ver);
		#else
				sprintf(des_buf, "YEJI-GT950-LXXXX-%x-%d",version_info,cfg_ver);
		#endif
	#endif
...
static void goodix_ts_suspend(struct goodix_ts_data *ts)
{
	...
	if(main_clock_init_lock)
	{
		GTP_INFO("In main clock init process no need syspend in ftm!");
		return;
	}
   ...
   if(ts->gtp_is_suspend)
   {
	...
	if(wake_up_switch)
	{
		   if(!wake_up_irq_status)
		   {
		   	GTP_INFO("Enable Irq wake up!");
		   	wake_up_irq_status=1;
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
    ...
    if (ts->enter_update || !ts->gtp_is_suspend) {
     ...
     if(wake_up_irq_status)
     {
     	     GTP_INFO("Dsiable Irq wake up!");
 	    wake_up_irq_status=0;
...
static int __init goodix_ts_init(void)
{
    ...
    {
	    GTP_INFO("GTP driver not installing in charger mode! \n");
		return -1;
	}

    if(strstr(saved_command_line,FTM_MODE_BOOT))
    {
    		 GTP_INFO("in ftm mode! \n");
			boot_mode=FTM_MODE;
    }else{
			boot_mode=0;
	}
	...
	{
		GTP_INFO("GTP driver has probe ok!! \n");
		return -1;
	}
    GTP_DEBUG_FUNC();   
