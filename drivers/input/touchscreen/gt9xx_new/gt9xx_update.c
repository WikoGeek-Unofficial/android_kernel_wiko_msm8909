...
extern void update_fw_version(u16 ver);
u16 max_vid=0;
#endif
//add end

#define TINNO_NOT_CHECK_FIRMWARE_BURN
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        GTP_ERROR("IT Don't meet the third condition.");
...
u8 gup_check_fs_mounted(char *path_name)
{
    ...
    if (err)
    {
        GTP_ERROR("%s NOT Mounted: %d", path_name, err);
		err = FAIL;
        goto exit_chk;
    }
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
    ...
    //step1:select bank2
    GTP_INFO("[download_fw_dsp]step1:select bank2");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x02);
    if (ret == FAIL)
    {
...
#ifndef TINNO_NOT_CHECK_FIRMWARE_BURN					
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
    ...
    for (section = 1; section <= 4; section += 2)
    {
        ...
        GTP_INFO("download firmware ss51: select bank%d", bank);
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head* fw_head, char* path)
{
    ...
    //check firmware legality
    fw_checksum = 0;
    for(i = FW_HEAD_LENGTH; i < sizeof(gtp_default_FW_fl); i += 2)
    {
...
s32 gup_load_by_addr(u16 addr, u8 need_check, u8 *fw, u32 length)
{
    ...
    {
        //switch to bank1
        gup_set_ic_msg(i2c_connect_client, _bRW_MISCTL__MEM_CD_EN, 1);
        ret = gup_set_ic_msg(i2c_connect_client, _bRW_MISCTL__SRAM_BANK, 1);    
        if (ret <= 0)
        {
        ...
        ret = i2c_write_bytes(i2c_connect_client, addr, fw, length);
        if (ret == -1)
        {
		...
		ret = gup_set_ic_msg(i2c_connect_client, _bRW_MISCTL__SRAM_BANK, 3);    
        if (ret <= 0)
        {
            GTP_ERROR("[load_by_addr]select bank fail.");
            continue;
        }
        gup_set_ic_msg(i2c_connect_client, _bRW_MISCTL__MEM_CD_EN, 0);
        break;
    }
    if(retry<5)
    {
...
void gup_load_init_code(void)
{
    ...
    gup_set_ic_msg(i2c_connect_client, _bRW_MISCTL__SRAM_BANK, 1);
    GTP_DEBUG("Write 0x01 to register 0x4049.");
    gup_set_ic_msg(i2c_connect_client, _bRW_MISCTL__MEM_CD_EN, 1);
    i2c_write_bytes(i2c_connect_client, 
                    0xF000, 
                    &gtp_default_FW_fl[FW_HEAD_LENGTH+0x7000], 
                    0x1000);
    gup_set_ic_msg(i2c_connect_client, 0x8040, 0x40);
}
...
s32 gup_load_overlay_system(u16 addr, u32 offset, u32 length)
{
    ...
    //test
    i2c_read_bytes(i2c_connect_client, 0x8041, &cmd, 1);
    ...
    i2c_write_bytes(i2c_connect_client, 0x8043, &cmd, 1);
    
    while(retry++ < 5)
    {
        msleep(10);
        ret = i2c_read_bytes(i2c_connect_client, 0x8043, &cmd, 1);
        if(ret < 0)
        {
            continue;
        }
    ...
    memcpy(buf, &gtp_default_FW_fl[FW_HEAD_LENGTH+offset], length);
#if GTP_ESD_PROTECT
    ...
    //close WDT
    cmd = 0x00;
    i2c_write_bytes(i2c_connect_client, 0x40B0, &cmd, 1);
    i2c_write_bytes(i2c_connect_client, 0x404B, &cmd, 1);
    i2c_write_bytes(i2c_connect_client, 0x40B4, &cmd, 1);
    i2c_write_bytes(i2c_connect_client, 0x40B8, &cmd, 1);
    //cmd = 0x08;
    //i2c_write_bytes(i2c_client_point, 0x4180, &cmd, 1);
    //close CLK
    i2c_read_bytes(i2c_connect_client, 0x4010, &cmd, 1);
    cmd |= 0x78;
    i2c_write_bytes(i2c_connect_client, 0x4010, &cmd, 1);
...
#if GTP_ESD_PROTECT
    cmd = 0xAA;
    i2c_write_bytes(i2c_connect_client, 0x8041 , &cmd, 1);
...
s32 gup_recovery_from_gesture(u8 type)
{
    ...
    firmware = &gtp_default_FW_fl[0x4800 + FW_HEAD_LENGTH];
    ...
    ret = i2c_write_bytes(i2c_connect_client, 0xC800, firmware, length);
    if (ret == -1)
    {
    ...
    if(type)
    {
        ...
        ret = gup_set_ic_msg(i2c_connect_client, _bRW_MISCTL__SRAM_BANK, 2);    
        if (ret <= 0)
        {
        ...
        ret = i2c_write_bytes(i2c_connect_client, 0xC000, firmware, length);
        if (ret == -1)
        {
