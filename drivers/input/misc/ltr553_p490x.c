...
//add for tp GESTURE mode
#if 0
static int gesture_ps_status=0;
#endif
//add end
...
static int ps_mode_flag=0; //0, call mode; 1, other mode
...
static int set_low_temperature_threshold_value(struct ltr553_data *obj,int ps)
{
	int temp_value_high;
	int temp_value_low;

	pr_info(" reset_low_temperature_threshold_value:  ps=%d \n",ps);
	pr_info("reset_low_temperature_threshold_value:reset_flag_low_temperature= %d  set_first_value_flag_low_temperature=%d \n", reset_flag_low_temperature,set_first_value_flag_low_temperature);
	...
	pr_info("set_low_temperature_threshold_value:g_ps_base_value= %d \n", g_ps_base_value);
	pr_info(" set_low_temperature_threshold_value:read  obj->default_ps_highthresh=%x, obj->default_ps_lowthresh=%x! \n", obj->default_ps_highthresh, obj->default_ps_lowthresh);
	...
	if(set_first_value_flag_low_temperature==0)
	{
		...
		else
		{
			...
			pr_info(" set_low_temperature_threshold_value:There is no need to set threshold! \n");
			obj->default_ps_highthresh = temp_value_high;
 			obj->default_ps_lowthresh = temp_value_low;
			goto end_set_threshold;		
		}
	...
	if( temp_value_high > 0x7ff)
	{
		...
		pr_info("set_low_temperature_threshold_value: set value_high=0x7ff, value_low=0x7ff, please check the phone \n");
	}
...
static uint16_t read_ps_adc_value_setthr(struct ltr553_data *ltr553)
{
	int8_t ret = -99;
	uint16_t value = -99;
	uint16_t ps_val;
...
#ifdef LOW_TEMPERATURE
	ps_val=set_low_temperature_threshold_value(ltr553,ps_val);
#endif
...
//add for tp
#if 0
	gesture_ps_status=psMode_set_reset;
#endif
//add end
...
static void report_ps_input_event(struct ltr553_data *ltr553)
{
	...
	if(ps_mode_flag ==0){
		adc_value = read_ps_adc_value_setthr(ltr553);
	}else{
...
#ifdef LOW_TEMPERATURE
				if((ps_mode_flag ==0) && (reset_flag_low_temperature == 0)){
                      	      buffer[0] = 0x92;
                      	      buffer[1] =  ltr553->default_ps_lowthresh&0xff;
                      	      ret = I2C_Write(buffer, 2);
                      	      if(ret < 0)
...
static int8_t ps_enable_init(struct ltr553_data *ltr553)
{
	...
	rc = set_ps_range(ltr553->default_ps_lowthresh, ltr553->default_ps_highthresh,
		 LO_N_HI_LIMIT, ltr553);
#else
...
static void ltr553_ps_cali_set_threshold(void)
{
    ...
    set_ps_range(ltr553->default_ps_lowthresh, ltr553->default_ps_highthresh,
    	 LO_N_HI_LIMIT, ltr553);
		...
		struct device_attribute *attr, char *buf)
{
	...
	value = read_ps_adc_value(ltr553);
	printk("%s value=%d, ltr553->default_ps_highthresh=%d,  ltr553->default_ps_lowthresh=%d ---CYY\n", __func__, value, ltr553->default_ps_highthresh,  ltr553->default_ps_lowthresh);
	 if(value > ltr553->default_ps_highthresh)
        {
               dat = 0;  /*close*/
        }
        else if(value < ltr553->default_ps_lowthresh)
        {
               dat = 1;  /*far*/
        }
	...
	msleep(20);
       ps_mode_setup(0, ltr553);


	return ret;
}

static DEVICE_ATTR(ps_value, 0444, ps_value_show, NULL);

//add by alik
 int ps_value_read(void)
{
#if 0
	uint16_t value;
	int dat;
	struct ltr553_data *ltr553 = sensor_info;
	if(gesture_ps_status==0)
	{
       	ps_mode_setup(1, ltr553);
		msleep(20);
	}
...
static ssize_t psmodeflag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int8_t ret = 0;
...
static ssize_t psmodeflag_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int param;
	struct ltr553_data *ltr553 = sensor_info;
...
static DEVICE_ATTR(ps_flag, 0644,
				psmodeflag_show, psmodeflag_store);
...
static DEVICE_ATTR(setalslothrerange, 0200, NULL, setalslothrerange_store);
...
static void sysfs_register_device(struct i2c_client *client)
{
	...
	rc += device_create_file(&client->dev, &dev_attr_ps_value);
	...
	rc += device_create_file(&client->dev, &dev_attr_ps_flag);
...
#ifdef CONFIG_TINNO_DEV_INFO
       CAREAT_TINNO_DEV_INFO(PSensor);
       SET_DEVINFO_STR(PSensor,"LTR579");
#endif
	return ret;
