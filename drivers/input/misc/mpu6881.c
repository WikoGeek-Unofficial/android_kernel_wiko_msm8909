/*
 * MPU6881 6-axis gyroscope + accelerometer driver
 *
...
DEF_TINNO_DEV_INFO(GYROSensor)
#endif
/*VDD 2.375V-3.46V VLOGIC 1.8V +-5%*/
#define MPU6881_VDD_MIN_UV		1710000
#define MPU6881_VDD_MAX_UV	3600000
#define MPU6881_VLOGIC_MIN_UV	1800000
#define MPU6881_VLOGIC_MAX_UV	1800000
#define MPU6881_VIO_MIN_UV	1710000
#define MPU6881_VIO_MAX_UV	3600000

#define MPU6881_ACCEL_MIN_VALUE	-32768
#define MPU6881_ACCEL_MAX_VALUE	32767
#define MPU6881_GYRO_MIN_VALUE	-32768
#define MPU6881_GYRO_MAX_VALUE	32767

//LINE<JIRA_ID><DATE20141201><modify delay time>zenghaihui
#define MPU6881_ACCEL_MIN_POLL_INTERVAL_MS	5
#define MPU6881_ACCEL_MAX_POLL_INTERVAL_MS	5000
#define MPU6881_ACCEL_DEFAULT_POLL_INTERVAL_MS	200

//LINE<JIRA_ID><DATE20141201><modify delay time>zenghaihui
#define MPU6881_GYRO_MIN_POLL_INTERVAL_MS	5
#define MPU6881_GYRO_MAX_POLL_INTERVAL_MS	5000
#define MPU6881_GYRO_DEFAULT_POLL_INTERVAL_MS	200

#define MPU6881_RAW_ACCEL_DATA_LEN	6
#define MPU6881_RAW_GYRO_DATA_LEN	6

/* Sensitivity Scale Factor */
#define MPU6881_ACCEL_SCALE_SHIFT_2G	0  ///4
#define MPU6881_GYRO_SCALE_SHIFT_FS0	3

#define MPU6881_DEV_NAME_ACCEL	"accelerometer"
#define MPU6881_DEV_NAME_GYRO		"gyroscope"
#define GYRO_RAW_DATA_FOR_CALI	_IOW('c', 3, int *)
#define GS_RAW_DATA_FOR_CALI	_IOW('c', 2, int *)
#define MPU6881_AXIS_X          0
#define MPU6881_AXIS_Y          1
#define MPU6881_AXIS_Z          2

#define MPU6881_AXES_NUM        3
/*#define MPU6881_FULL_RES			0x08*/
#define MPU6881_RANGE_2G			(0x00 << 3)
#define MPU6881_RANGE_4G			(0x01 << 3)
#define MPU6881_RANGE_8G			(0x02 << 3)
#define MPU6881_RANGE_16G			(0x03 << 3)
/*#define MPU6881_SELF_TEST         0x80*/
#define MPU6881_REG_DATA_FORMAT		0x1C

#define MPU6881_RANGE_PN250dps			(0x00 << 3)
#define MPU6881_RANGE_PN500dps			(0x01 << 3)
#define MPU6881_RANGE_PN1000dps			(0x02 << 3)
#define MPU6881_RANGE_PN2000dps			(0x03 << 3)
#define MPU6881_REG_Gyro_DATA_FORMAT		0x1B
...
enum mpu6881_place {
	...
	MPU6881_PLACE_LD = 2,
	MPU6881_PLACE_LL = 3,
	MPU6881_PLACE_PU_BACK = 4,
	MPU6881_PLACE_PR_BACK = 5,
	MPU6881_PLACE_LD_BACK = 6,
	MPU6881_PLACE_LL_BACK = 7,
	MPU6881_PLACE_UNKNOWN = 8,
	MPU6881_AXIS_REMAP_TAB_SZ = 8
};

struct mpu6881_place_name {
	char name[32];
	enum mpu6881_place place;
};
...
/**
 *  struct mpu6881_sensor - Cached chip configuration data.
 *  @client:		I2C client.
...
struct mpu6881_sensor {
	...
	struct mpu6881_platform_data *pdata;
...
struct mpu6881_sensor *mpu_info;
/* Accelerometer information read by HAL */
static struct sensors_classdev mpu6881_acc_cdev = {
	.name = "MPU6881-accel",
	.vendor = "Invensense",
	.version = 1,
	...
	.min_delay = MPU6881_ACCEL_MIN_POLL_INTERVAL_MS * 1000,
	.delay_msec = MPU6881_ACCEL_DEFAULT_POLL_INTERVAL_MS,
...
static struct sensors_classdev mpu6881_gyro_cdev = {
	.name = "MPU6881-gyro",
	.vendor = "Invensense",
	.version = 1,
	...
	.min_delay = MPU6881_GYRO_MIN_POLL_INTERVAL_MS * 1000,
	.delay_msec = MPU6881_ACCEL_DEFAULT_POLL_INTERVAL_MS,
	.fifo_reserved_event_count = 0,
	.fifo_max_event_count = 0,
	.enabled = 0,
...
mpu6881_accel_axis_remap_tab[MPU6881_AXIS_REMAP_TAB_SZ] = {
...
static const struct sensor_axis_remap
mpu6881_gyro_axis_remap_tab[MPU6881_AXIS_REMAP_TAB_SZ] = {
	/* src_x src_y src_z  sign_x  sign_y  sign_z */
	{  0,    1,    2,    -1,      1,     -1 }, /* P0 */
	{  1,    0,    2,    -1,     -1,     -1 }, /* P1*/
	{  0,    1,    2,     1,     -1,     -1 }, /* P2 */
...
static const struct mpu6881_place_name
mpu6881_place_name2num[MPU6881_AXIS_REMAP_TAB_SZ] = {
	{"Portrait Up", MPU6881_PLACE_PU},
	{"Landscape Right", MPU6881_PLACE_PR},
	{"Portrait Down", MPU6881_PLACE_LD},
	{"Landscape Left", MPU6881_PLACE_LL},
	{"Portrait Up Back Side", MPU6881_PLACE_PU_BACK},
	{"Landscape Right Back Side", MPU6881_PLACE_PR_BACK},
	{"Portrait Down Back Side", MPU6881_PLACE_LD_BACK},
	{"Landscape Left Back Side", MPU6881_PLACE_LL_BACK},
};

static void setup_mpu6881_reg(struct mpu_reg_map *reg);
static int mpu6881_init_config(struct mpu6881_sensor *sensor);

static int mpu6881_power_ctl(struct mpu6881_sensor *sensor, bool on)
{
	int rc = 0;
...
static int mpu6881_power_init(struct mpu6881_sensor *sensor)
{
	int ret = 0;
	...
	if (regulator_count_voltages(sensor->vdd) > 0) {
		ret = regulator_set_voltage(sensor->vdd, MPU6881_VDD_MIN_UV,
					   MPU6881_VDD_MAX_UV);
		if (ret) {
			dev_err(&sensor->client->dev,
...
reg_vdd_set_vtg:
	if (regulator_count_voltages(sensor->vdd) > 0)
		regulator_set_voltage(sensor->vdd, 0, MPU6881_VDD_MAX_UV);
reg_vdd_put:
	regulator_put(sensor->vdd);
	return ret;
}

static int mpu6881_power_deinit(struct mpu6881_sensor *sensor)
{
	int ret = 0;
	if (regulator_count_voltages(sensor->vdd) > 0)
		regulator_set_voltage(sensor->vdd, 0, MPU6881_VDD_MAX_UV);
...
/**
 * mpu6881_read_reg - read multiple register data
 ...
 */
static int mpu6881_read_reg(struct i2c_client *client, u8 start_addr,
...
static int8_t I2C_Write(uint8_t *txData, uint8_t length)
{
	...
	struct mpu6881_sensor *self_info = mpu_info;
	struct i2c_msg data[] = {
		{
...
static int mpu6881_write_reg(struct i2c_client *client, u8 start_addr,
			       u8 data, int length)
{
	int ret = 0;
	u8 buf[2];
...
/**
 * mpu6881_read_accel_data - get accelerometer data from device
 * @sensor: sensor device instance
 * @data: axis data to update
 *
 * Return the converted X Y and Z data from the sensor device
 */
static void mpu6881_read_accel_data(struct mpu6881_sensor *sensor,
			     struct axis_data *data)
{
	u16 buffer[3];

	mpu6881_read_reg(sensor->client, sensor->reg.raw_accel,
		(u8 *)buffer, MPU6881_RAW_ACCEL_DATA_LEN);
...
/**
 * mpu6881_read_gyro_data - get gyro data from device
 * @sensor: sensor device instance
 * @data: axis data to update
 *
 * Return the converted RX RY and RZ data from the sensor device
 */
static void mpu6881_read_gyro_data(struct mpu6881_sensor *sensor,
			     struct axis_data *data)
{
	u16 buffer[3];

	mpu6881_read_reg(sensor->client, sensor->reg.raw_gyro,
		(u8 *)buffer, MPU6881_RAW_GYRO_DATA_LEN);
...
/**
 * mpu6881_remap_accel_data - remap accelerometer raw data to axis data
 * @data: data needs remap
 * @place: sensor position
 */
static void mpu6881_remap_accel_data(struct axis_data *data, int place)
{
	const struct sensor_axis_remap *remap;
	s16 tmp[3];
	/* sensor with place 0 needs not to be remapped */
        //LINE<JIRA_ID><DATE20141202><mpu6881 axis remap>zenghaihui
	//if ((place <= 0) || (place >= MPU6881_AXIS_REMAP_TAB_SZ))
	if ((place < 0) || (place >= MPU6881_AXIS_REMAP_TAB_SZ))
		return;

	remap = &mpu6881_accel_axis_remap_tab[place];
...
/**
 * mpu6881_remap_gyro_data - remap gyroscope raw data to axis data
 * @data: data needs remap
 * @place: sensor position
 */
static void mpu6881_remap_gyro_data(struct axis_data *data, int place)
{
	const struct sensor_axis_remap *remap;
	s16 tmp[3];
	/* sensor with place 0 needs not to be remapped */
	...
	remap = &mpu6881_gyro_axis_remap_tab[place];
	tmp[0] = data->rx;
	tmp[1] = data->ry;
	tmp[2] = data->rz;
...
/**
 * mpu6881_interrupt_thread - handle an IRQ
 ...
 */
static irqreturn_t mpu6881_interrupt_thread(int irq, void *data)
{
	struct mpu6881_sensor *sensor = data;

	mpu6881_read_accel_data(sensor, &sensor->axis);
	mpu6881_read_gyro_data(sensor, &sensor->axis);
...
/**
 * mpu6881_accel_work_fn - polling accelerometer data
 * @work: the work struct
 *
 * Called by the work queue; read sensor data and generate an input
 * event
 */
static void mpu6881_accel_work_fn(struct work_struct *work)
{
	struct mpu6881_sensor *sensor;
	ktime_t timestamp;
	
	sensor = container_of((struct delayed_work *)work,
				struct mpu6881_sensor, accel_poll_work);

	mpu6881_read_accel_data(sensor, &sensor->axis);
	mpu6881_remap_accel_data(&sensor->axis, sensor->pdata->place);

	timestamp = ktime_get_boottime();
	input_report_abs(sensor->accel_dev, ABS_X,
		(sensor->axis.x >> MPU6881_ACCEL_SCALE_SHIFT_2G));
	input_report_abs(sensor->accel_dev, ABS_Y,
		(sensor->axis.y >> MPU6881_ACCEL_SCALE_SHIFT_2G));
	input_report_abs(sensor->accel_dev, ABS_Z,
		(sensor->axis.z >> MPU6881_ACCEL_SCALE_SHIFT_2G));
...
/**
 * mpu6881_gyro_work_fn - polling gyro data
 * @work: the work struct
 *
 * Called by the work queue; read sensor data and generate an input
 * event
 */
static void mpu6881_gyro_work_fn(struct work_struct *work)
{
	struct mpu6881_sensor *sensor;
	ktime_t timestamp;
	
	sensor = container_of((struct delayed_work *)work,
				struct mpu6881_sensor, gyro_poll_work);

	mpu6881_read_gyro_data(sensor, &sensor->axis);
	mpu6881_remap_gyro_data(&sensor->axis, sensor->pdata->place);
...
/**
 *  mpu6881_set_lpa_freq() - set low power wakeup frequency.
 */
static int mpu6881_set_lpa_freq(struct mpu6881_sensor *sensor, int lpa_freq)
{
	int ret;
	u8 data;

	/* only for MPU6881 with fixed rate, need expend */
	if (INV_MPU6881 == sensor->chip_type) {
...
static int mpu6881_switch_engine(struct mpu6881_sensor *sensor,
				bool en, u32 mask)
{
	struct mpu_reg_map *reg;
	u8 data, mgmt_1;
...
static int mpu6881_init_engine(struct mpu6881_sensor *sensor)
{
	int ret;

	ret = mpu6881_switch_engine(sensor, false, BIT_PWR_GYRO_STBY_MASK);
	if (ret)
		return ret;

	ret = mpu6881_switch_engine(sensor, false, BIT_PWR_ACCEL_STBY_MASK);
	if (ret)
		return ret;
...
/**
 * mpu6881_set_power_mode - set the power mode
 ...
 */
static int mpu6881_set_power_mode(struct mpu6881_sensor *sensor,
					bool power_on)
{
...
static int mpu6881_gyro_enable(struct mpu6881_sensor *sensor, bool on)
{
	int ret;
	u8 data;
	...
	data = (u8)ret;
	if (on) {
		ret = mpu6881_switch_engine(sensor, true,
		...
		ret = mpu6881_switch_engine(sensor, false,
			BIT_PWR_GYRO_STBY_MASK);
		if (ret)
			return ret;
...
/**
 * mpu6881_restore_context - update the sensor register context
 */

static int mpu6881_restore_context(struct mpu6881_sensor *sensor)
{
	struct mpu_reg_map *reg;
	struct i2c_client *client;
	int ret;
	...
	ret = mpu6881_set_lpa_freq(sensor, sensor->cfg.lpa_freq);
	if (ret < 0) {
...
/**
 * mpu6881_reset_chip - reset chip to default state
 */
static void mpu6881_reset_chip(struct mpu6881_sensor *sensor)
{
	struct i2c_client *client;
	int ret, i;
	...
	for (i = 0; i < MPU6881_RESET_RETRY_CNT; i++) {
		ret = i2c_smbus_read_byte_data(sensor->client,
					sensor->reg.pwr_mgmt_1);
		if (ret < 0) {
			dev_err(&sensor->client->dev,
...
static int mpu6881_gyro_set_enable(struct mpu6881_sensor *sensor, bool enable)
{
	int ret = 0;
	...
	if (enable) {
		if (!sensor->cfg.enable) {
			ret = mpu6881_power_ctl(sensor, true);
			if (ret < 0) {
				dev_err(&sensor->client->dev,
						"Failed to power up mpu6881\n");
				goto exit;
			}
			ret = mpu6881_set_power_mode(sensor, true);
			ret = mpu6881_restore_context(sensor);
			if (ret < 0) {
				dev_err(&sensor->client->dev,
		...
		ret = mpu6881_gyro_enable(sensor, true);
		if (ret) {
			dev_err(&sensor->client->dev,
		...
		ret = mpu6881_gyro_enable(sensor, false);
		if (ret) {
			dev_err(&sensor->client->dev,
...
static int mpu6881_gyro_set_poll_delay(struct mpu6881_sensor *sensor,
					unsigned long delay)
{
	mutex_lock(&sensor->op_lock);
	if (delay < MPU6881_GYRO_MIN_POLL_INTERVAL_MS)
		delay = MPU6881_GYRO_MIN_POLL_INTERVAL_MS;
	if (delay > MPU6881_GYRO_MAX_POLL_INTERVAL_MS)
		delay = MPU6881_GYRO_MAX_POLL_INTERVAL_MS;
...
static int mpu6881_gyro_cdev_enable(struct sensors_classdev *sensors_cdev,
			unsigned int enable)
{
	struct mpu6881_sensor *sensor = container_of(sensors_cdev,
			struct mpu6881_sensor, gyro_cdev);

	return mpu6881_gyro_set_enable(sensor, enable);
}

static int mpu6881_gyro_cdev_poll_delay(struct sensors_classdev *sensors_cdev,
			unsigned int delay_ms)
{
	struct mpu6881_sensor *sensor = container_of(sensors_cdev,
			struct mpu6881_sensor, gyro_cdev);

	return mpu6881_gyro_set_poll_delay(sensor, delay_ms);
}

/**
 * mpu6881_gyro_attr_get_polling_delay - get the sampling rate
 */
static ssize_t mpu6881_gyro_attr_get_polling_delay(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	int val;
	struct mpu6881_sensor *sensor = dev_get_drvdata(dev);
...
/**
 * mpu6881_gyro_attr_set_polling_delay - set the sampling rate
 */
static ssize_t mpu6881_gyro_attr_set_polling_delay(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	...
	ret = mpu6881_gyro_set_poll_delay(sensor, interval_ms);

	return ret ? -EBUSY : size;
}

static int MPU6881_SetPowerMode(struct i2c_client *iclient)
{
	int i = 0;
	u8  temp_data;
	struct mpu6881_sensor *self_info = mpu_info;
	...
	for (; i <= 107; i++) {
		mpu6881_read_reg(self_info->client, i, &temp_data, 1);
		/*pr_info("wlg_set_self_test----read 0x%d,%X\n",i,temp_data);*/
	}

	mpu6881_write_reg(self_info->client, 0X38, 0X00, 1);
	mpu6881_write_reg(self_info->client, 0X23, 0X00, 1);
	mpu6881_write_reg(self_info->client, 0X6A, 0X00, 1);
	mpu6881_write_reg(self_info->client, 0X6A, 0X04, 1);
	mpu6881_write_reg(self_info->client, 0X1A, 0X02, 1);
	mpu6881_write_reg(self_info->client, 0X1D, 0X02, 1);
	mpu6881_write_reg(self_info->client, 0X19, 0X00, 1);
	mpu6881_write_reg(self_info->client, 0X1B, 0X00, 1);
	mpu6881_write_reg(self_info->client, 0X1C, 0X00, 1);
	mpu6881_write_reg(self_info->client, 0X6A, 0X40, 1);
	mpu6881_write_reg(self_info->client, 0X6B, 0X0, 1);
	mpu6881_write_reg(self_info->client, 0X6C, 0X0, 1);
	mpu6881_write_reg(self_info->client, 0X23, 0X78, 1);
	mpu6881_write_reg(self_info->client, 0X23, 0X00, 1);

	return 0;
}

static int MPU6881_JudgeTestResult(struct i2c_client *client,
s32 prv[MPU6881_AXES_NUM], s32 nxt[MPU6881_AXES_NUM])
{
	struct criteria {
		int min;
		int max;
	...
	/*mpu6881_read_reg(sensor->client,
	sensor->reg.raw_gyro,(u8 *)buffer,
	MPU6881_RAW_GYRO_DATA_LEN);*/

	res = mpu6881_read_reg(client, MPU6881_REG_Gyro_DATA_FORMAT,
	&format, 1);
	if (res < 0)
		return res;
	else
	...
	format = format & MPU6881_RANGE_PN2000dps;

	switch (format) {
	case MPU6881_RANGE_PN250dps:
		pr_info("format use gyro_offset[0]\n");
		ptr = &gyro_offset[0];
		break;

	case MPU6881_RANGE_PN500dps:
		pr_info("format use gyro_offset[1]\n");
		ptr = &gyro_offset[1];
		break;

	case MPU6881_RANGE_PN1000dps:
		pr_info("format use gyro_offset[2]\n");
		ptr = &gyro_offset[2];
		break;

	case MPU6881_RANGE_PN2000dps:
	...
	pr_info("X diff is %ld\n",
		abs(nxt[MPU6881_AXIS_X] - prv[MPU6881_AXIS_X]));
	pr_info("Y diff is %ld\n",
		abs(nxt[MPU6881_AXIS_Y] - prv[MPU6881_AXIS_Y]));
	pr_info("Z diff is %ld\n",
		abs(nxt[MPU6881_AXIS_Z] - prv[MPU6881_AXIS_Z]));

	if (abs(prv[MPU6881_AXIS_X]) > (*ptr)[MPU6881_AXIS_X].max) {
		pr_info("gyro X offset[%X] is over range\n",
			prv[MPU6881_AXIS_X]);
		res = -EINVAL;
	}

	if (abs(prv[MPU6881_AXIS_Y]) > (*ptr)[MPU6881_AXIS_Y].max) {
		pr_info("gyro Y offset[%X] is over range\n",
			prv[MPU6881_AXIS_Y]);
		res = -EINVAL;
	}

	if (abs(prv[MPU6881_AXIS_Z]) > (*ptr)[MPU6881_AXIS_Z].max) {
		pr_info("gyro Z offset[%X] is over range\n",
			prv[MPU6881_AXIS_Z]);
		res = -EINVAL;
	}

	if ((abs(nxt[MPU6881_AXIS_X] - prv[MPU6881_AXIS_X])
		< (*ptr)[MPU6881_AXIS_X].min)) {
		pr_info("X is out of work\n");
		res = -EINVAL;
	}

	if ((abs(nxt[MPU6881_AXIS_Y] - prv[MPU6881_AXIS_Y])
		< (*ptr)[MPU6881_AXIS_Y].min)) {
		pr_info("Y is out of work\n");
		res = -EINVAL;
	}

	if ((abs(nxt[MPU6881_AXIS_Z] - prv[MPU6881_AXIS_Z])
		< (*ptr)[MPU6881_AXIS_Z].min)) {
...
struct device_attribute *attr, const char *buf, size_t count)
{
	...
	int idx, num;
	int ret = 0;
	s32 avg_prv[MPU6881_AXES_NUM] = {0, 0, 0};
	s32 avg_nxt[MPU6881_AXES_NUM] = {0, 0, 0};
	...
	pr_info("NORMAL:\n");
	MPU6881_SetPowerMode(self_info->client);
	msleep(50);

	for (idx = 0; idx < num; idx++) {
		/*mpu6881_read_accel_data(self_info, self_data);*/
		/*wlg test gyro self test*/
		mpu6881_read_gyro_data(self_info, self_data);

		pr_info("x = %d,y = %d,z= %d\n", self_data->rx,
			self_data->ry, self_data->rz);
		avg_prv[MPU6881_AXIS_X] += self_data->rx;
		avg_prv[MPU6881_AXIS_Y] += self_data->ry;
		avg_prv[MPU6881_AXIS_Z] += self_data->rz;
		pr_info("[%5d %5d %5d]\n", self_data->rx,
			self_data->ry, self_data->rz);
	}

	avg_prv[MPU6881_AXIS_X] /= num;
	avg_prv[MPU6881_AXIS_Y] /= num;
	avg_prv[MPU6881_AXIS_Z] /= num;

	/*initial setting for self test*/
	pr_info("SELFTEST:\n");
	/*wlg test gyro self test [write 0E to 1B enable self test]*/
	mpu6881_read_reg(self_info->client, 0X1B, &temp_data, 1);
	pr_info("wlg_set_self_test----read 0x1B    %d\n", temp_data);
	temp_data |= 0xE0;
	/*mpu6881_write_reg(self_info->client, 0X1B, temp_data, 1);*/
	...
	for (idx = 0; idx < num; idx++) {
		mpu6881_read_gyro_data(self_info, self_data2);

		pr_info("xx = %d,yy = %d,zz= %d\n", self_data2->rx,
			self_data2->ry, self_data2->rz);
		avg_nxt[MPU6881_AXIS_X] += self_data2->rx;
		avg_nxt[MPU6881_AXIS_Y] += self_data2->ry;
		avg_nxt[MPU6881_AXIS_Z] += self_data2->rz;
		pr_info("[%5d %5d %5d]\n", self_data2->rx,
			self_data2->ry, self_data2->rz);
	}

	avg_nxt[MPU6881_AXIS_X] /= num;
	avg_nxt[MPU6881_AXIS_Y] /= num;
	avg_nxt[MPU6881_AXIS_Z] /= num;

	pr_info("X: %5d - %5d = %5d\n", avg_nxt[MPU6881_AXIS_X],
		avg_prv[MPU6881_AXIS_X],
		avg_nxt[MPU6881_AXIS_X] - avg_prv[MPU6881_AXIS_X]);
	pr_info("Y: %5d - %5d = %5d\n", avg_nxt[MPU6881_AXIS_Y],
		avg_prv[MPU6881_AXIS_Y],
		avg_nxt[MPU6881_AXIS_Y] - avg_prv[MPU6881_AXIS_Y]);
	pr_info("Z: %5d - %5d = %5d\n", avg_nxt[MPU6881_AXIS_Z],
		avg_prv[MPU6881_AXIS_Z],
	avg_nxt[MPU6881_AXIS_Z] - avg_prv[MPU6881_AXIS_Z]);
/*
#if 1
	if (!MPU6881_JudgeTestResult(self_info->client,
		avg_prv, avg_nxt))
#else
	if (1)
#endif
...
static ssize_t mpu6881_gyro_attr_get_enable(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct mpu6881_sensor *sensor = dev_get_drvdata(dev);
 ...
 *    Set/get enable function is just needed by sensor HAL.
 */
static ssize_t mpu6881_gyro_attr_set_enable(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	...
	if (enable)
		ret = mpu6881_gyro_set_enable(sensor, true);
	else
		ret = mpu6881_gyro_set_enable(sensor, false);
...
static struct device_attribute gyro_attr[] = {
	...
	mpu6881_gyro_attr_get_polling_delay,
	mpu6881_gyro_attr_set_polling_delay),
	__ATTR(selftest, S_IRUGO | S_IWUSR,
	show_self_value, store_self_value),
	__ATTR(enable, S_IRUGO | S_IWUSR,
	mpu6881_gyro_attr_get_enable,
	mpu6881_gyro_attr_set_enable),
};
...
static int mpu6881_accel_enable(struct mpu6881_sensor *sensor, bool on)
{
	int ret;
	u8 data;
...
static int mpu6881_accel_set_enable(struct mpu6881_sensor *sensor, bool enable)
{
	int ret = 0;
	...
	if (enable) {
		if (!sensor->cfg.enable) {
			...
			if (ret < 0) {
				dev_err(&sensor->client->dev,
					"Failed to set power up mpu6881");
				goto exit;
			}
		...
		ret = mpu6881_accel_enable(sensor, true);
		if (ret) {
			dev_err(&sensor->client->dev,
		...
		ret = mpu6881_accel_enable(sensor, false);
		if (ret) {
			dev_err(&sensor->client->dev,
...
static int mpu6881_accel_set_poll_delay(struct mpu6881_sensor *sensor,
					unsigned long delay)
{
	u8 divider;
	int ret;
	...
	mutex_lock(&sensor->op_lock);
	if (delay < MPU6881_ACCEL_MIN_POLL_INTERVAL_MS)
		delay = MPU6881_ACCEL_MIN_POLL_INTERVAL_MS;
	if (delay > MPU6881_ACCEL_MAX_POLL_INTERVAL_MS)
		delay = MPU6881_ACCEL_MAX_POLL_INTERVAL_MS;
...
static int mpu6881_accel_cdev_enable(struct sensors_classdev *sensors_cdev,
			unsigned int enable)
{
	struct mpu6881_sensor *sensor = container_of(sensors_cdev,
			struct mpu6881_sensor, accel_cdev);

	return mpu6881_accel_set_enable(sensor, enable);
}

static int mpu6881_accel_cdev_poll_delay(struct sensors_classdev *sensors_cdev,
			unsigned int delay_ms)
{
	struct mpu6881_sensor *sensor = container_of(sensors_cdev,
			struct mpu6881_sensor, accel_cdev);

	return mpu6881_accel_set_poll_delay(sensor, delay_ms);
}

/**
 * mpu6881_accel_attr_get_polling_delay - get the sampling rate
 */
static ssize_t mpu6881_accel_attr_get_polling_delay(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	int val;
...
/**
 * mpu6881_accel_attr_set_polling_delay - set the sampling rate
 */
static ssize_t mpu6881_accel_attr_set_polling_delay(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct mpu6881_sensor *sensor = dev_get_drvdata(dev);
	...
	ret = mpu6881_accel_set_poll_delay(sensor, interval_ms);

	return ret ? -EBUSY : size;
}

static ssize_t mpu6881_accel_attr_get_enable(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct mpu6881_sensor *sensor = dev_get_drvdata(dev);
...
/**
 * mpu6881_accel_attr_set_enable -
 *    Set/get enable function is just needed by sensor HAL.
 */

static ssize_t mpu6881_accel_attr_set_enable(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	struct mpu6881_sensor *sensor = dev_get_drvdata(dev);
	...
	if (enable)
		ret = mpu6881_accel_set_enable(sensor, true);
	else
		ret = mpu6881_accel_set_enable(sensor, false);
...
static ssize_t mpu6881_accel_attr_get_reg_addr(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	return snprintf(buf, 8, "%d\n", mpu6881_address);
...
static ssize_t mpu6881_accel_attr_set_reg_addr(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	unsigned long addr;
	...
	mpu6881_address = addr;
	dev_info(dev, "mpu6881_address =%d\n", mpu6881_address);

	return size;
}

static ssize_t mpu6881_accel_attr_get_data(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	struct mpu6881_sensor *sensor = dev_get_drvdata(dev);
	...
	ret = i2c_smbus_read_byte_data(sensor->client, mpu6881_address);
...
static ssize_t mpu6881_accel_attr_set_data(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	...
	mpu6881_data = reg_data;
	dev_info(dev, "set mpu6881_data =0x%x\n", mpu6881_data);

	return size;
}
static ssize_t mpu6881_accel_attr_reg_write(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct mpu6881_sensor *sensor = dev_get_drvdata(dev);
	...
	ret = i2c_smbus_write_byte_data(sensor->client,
		mpu6881_address, mpu6881_data);
	dev_info(dev, "write addr(0x%x)<-0x%x ret=%d\n",
		mpu6881_address, mpu6881_data, ret);
...
mpu6881_accel_attr_get_polling_delay,
mpu6881_accel_attr_set_polling_delay),
__ATTR(enable, S_IRUGO | S_IWUSR,
mpu6881_accel_attr_get_enable,
mpu6881_accel_attr_set_enable),
#ifdef DEBUG_NODE
	__ATTR(addr, S_IRUSR | S_IWUSR,
		mpu6881_accel_attr_get_reg_addr,
		mpu6881_accel_attr_set_reg_addr),
	__ATTR(reg, S_IRUSR | S_IWUSR,
		mpu6881_accel_attr_get_data,
		mpu6881_accel_attr_set_data),
	__ATTR(write, S_IWUSR,
		NULL,
		mpu6881_accel_attr_reg_write),
#endif
};
 ...
 */
static int mpu_check_chip_type(struct mpu6881_sensor *sensor,
		const struct i2c_device_id *id)
{
	...
	if (!strcmp(id->name, "mpu6881"))
			sensor->chip_type = INV_MPU6881;
		...
		return -EPERM;
	reg = &sensor->reg;
	setup_mpu6881_reg(reg);

	/* turn off and turn on power to ensure gyro engine is on */

	ret = mpu6881_set_power_mode(sensor, false);
	if (ret)
		return ret;
	ret = mpu6881_set_power_mode(sensor, true);
	if (ret)
...
/**
 *  mpu6881_init_config() - Initialize hardware, disable FIFO.
...
#ifdef CONFIG_OF
static int mpu6881_dt_get_place(struct device *dev,
			struct mpu6881_platform_data *pdata)
{
	...
	for (i = 0; i < MPU6881_AXIS_REMAP_TAB_SZ; i++) {
		if (!strcmp(place_name, mpu6881_place_name2num[i].name)) {
			pdata->place = mpu6881_place_name2num[i].place;
			break;
		}
	}
	if (i >= MPU6881_AXIS_REMAP_TAB_SZ) {
...
static int mpu6881_parse_dt(struct device *dev,
			struct mpu6881_platform_data *pdata)
{
	int rc;

	rc = mpu6881_dt_get_place(dev, pdata);
	if (rc)
		return rc;
...
/**
 * mpu6881_probe - device detection callback
...
/* GS open fops */
int gs_open(struct inode *inode, struct file *file)
{
...
/* GS release fops */
int gs_release(struct inode *inode, struct file *file)
{
	return 0;
}
...
static long gs_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct mpu6881_sensor *sensor = file->private_data;
	struct cali_data rawdata;

	switch (cmd) {
	case GS_RAW_DATA_FOR_CALI:
			mpu6881_read_accel_data(sensor, &sensor->axis);
			mpu6881_remap_accel_data(&sensor->axis,
				sensor->pdata->place);
...
int gyro_open(struct inode *inode, struct file *file)
{
...
/* GS release fops */
int gyro_release(struct inode *inode, struct file *file)
{
	return 0;
}
...
static long gyro_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GYRO_RAW_DATA_FOR_CALI:
			mpu6881_read_gyro_data(sensor, &sensor->axis);
			mpu6881_remap_gyro_data(&sensor->axis,
				sensor->pdata->place);
...
static int mpu6881_probe(struct i2c_client *client,
				   const struct i2c_device_id *id)
{
	struct mpu6881_sensor *sensor;
	struct mpu6881_platform_data *pdata;
	...
	sensor = devm_kzalloc(&client->dev, sizeof(struct mpu6881_sensor),
			GFP_KERNEL);
	if (!sensor) {
	...
	if (client->dev.of_node) {
		pdata = devm_kzalloc(&client->dev,
			sizeof(struct mpu6881_platform_data), GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev, "Failed to allcated memory\n");
			ret = -ENOMEM;
			goto err_free_devmem;
		}
		ret = mpu6881_parse_dt(&client->dev, pdata);
		if (ret) {
	...
	ret = mpu6881_init_engine(sensor);
	if (ret) {
		dev_err(&client->dev, "Failed to init chip engine\n");
		goto err_power_off_device;
	}

	ret = mpu6881_set_lpa_freq(sensor, MPU6881_LPA_5HZ);
	if (ret) {
	...
	ret = mpu6881_init_config(sensor);
	if (ret) {
	...
	sensor->accel_dev->name = MPU6881_DEV_NAME_ACCEL;
	sensor->gyro_dev->name = MPU6881_DEV_NAME_GYRO;
	...
	input_set_abs_params(sensor->accel_dev, ABS_X,
			MPU6881_ACCEL_MIN_VALUE, MPU6881_ACCEL_MAX_VALUE,
			0, 0);
	...
	input_set_abs_params(sensor->gyro_dev, ABS_RX,
			     MPU6881_GYRO_MIN_VALUE, MPU6881_GYRO_MAX_VALUE,
			     0, 0);
	...
	if ((sensor->pdata->use_int) &&
		...
		sensor->use_poll = 1;
			...
			mpu6881_gyro_work_fn);
		INIT_DELAYED_WORK(&sensor->accel_poll_work,
			mpu6881_accel_work_fn);
		dev_dbg(&client->dev,
	...
	sensor->gyro_cdev = mpu6881_gyro_cdev;
	sensor->gyro_cdev.delay_msec = sensor->gyro_poll_ms;
	sensor->gyro_cdev.sensors_enable = mpu6881_gyro_cdev_enable;
	sensor->gyro_cdev.sensors_poll_delay = mpu6881_gyro_cdev_poll_delay;
	...
	sensor->accel_cdev = mpu6881_acc_cdev;
	sensor->accel_cdev.delay_msec = sensor->accel_poll_ms;
	sensor->accel_cdev.sensors_enable = mpu6881_accel_cdev_enable;
	sensor->accel_cdev.sensors_poll_delay = mpu6881_accel_cdev_poll_delay;
	...
	ret = mpu6881_power_ctl(sensor, false);
	if (ret) {
		dev_err(&client->dev,
				"Power off mpu6881 failed\n");
		goto err_remove_accel_cdev;
	}
#ifdef CONFIG_TINNO_DEV_INFO
       CAREAT_TINNO_DEV_INFO(GYROSensor);
       SET_DEVINFO_STR(GYROSensor,"MPU6881");
#endif
	return 0;
...
err_power_off_device:
	mpu6881_power_ctl(sensor, false);
err_deinit_regulator:
	mpu6881_power_deinit(sensor);
...
/**
 * mpu6881_remove - remove a sensor
 * @client: i2c client of sensor being removed
 *
 * Our sensor is going away, clean up the resources.
 */
static int mpu6881_remove(struct i2c_client *client)
{
	struct mpu6881_sensor *sensor = i2c_get_clientdata(client);
...
#ifdef CONFIG_PM
/**
 * mpu6881_suspend - called on device suspend
 * @dev: device being suspended
 *
 * Put the device into sleep mode before we suspend the machine.
 */
static int mpu6881_suspend(struct device *dev)
{
	...
	mpu6881_set_power_mode(sensor, false);
	ret = mpu6881_power_ctl(sensor, false);
	if (ret < 0) {
...
/**
 * mpu6881_resume - called on device resume
 * @dev: device being resumed
 *
 * Put the device into powered mode on resume.
 */
static int mpu6881_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct mpu6881_sensor *sensor = i2c_get_clientdata(client);
	int ret = 0;
	...
	ret = mpu6881_power_ctl(sensor, true);
	if (ret < 0) {
		dev_err(&client->dev, "Power on mpu6881 failed\n");
		goto exit;
	}
	/* Reset sensor to recovery from unexpect state */
	mpu6881_reset_chip(sensor);
	...
	if (sensor->cfg.enable) {
		...
		mpu6881_set_power_mode(sensor, true);
	} else {
		mpu6881_set_power_mode(sensor, false);
	}
...
static UNIVERSAL_DEV_PM_OPS(mpu6881_pm, mpu6881_suspend, mpu6881_resume, NULL);

static const struct i2c_device_id mpu6881_ids[] = {
	{ "mpu6881", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, mpu6881_ids);

static const struct of_device_id mpu6881_of_match[] = {
	{ .compatible = "invn,mpu6881", },
	{ },
};
MODULE_DEVICE_TABLE(of, mpu6881_of_match);

static struct i2c_driver mpu6881_i2c_driver = {
	.driver	= {
		.name	= "mpu6881",
		.owner	= THIS_MODULE,
		.pm	= &mpu6881_pm,
		.of_match_table = mpu6881_of_match,
	},
	.probe		= mpu6881_probe,
	.remove		= mpu6881_remove,
